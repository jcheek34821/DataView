...
def export_to_mp4():
    ...
    path_count = 1
    while os.path.exists(hard_drive + path_to_videos + mp4string):
        mp4string = "new_video(" + str(path_count) + ").mp4"
        path_count += 1

    # choose codec according to format needed
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    video = cv2.VideoWriter(hard_drive + path_to_videos + mp4string, fourcc, 30, (width, height))
...
def remove_rows():
    df = data_frame[0].winfo_children()
    for x in df:
        if df.index(x) > 0:
            x.destroy()
...
def filter_col(regex):
    ...
    for x in value_options:
        if len(filter_row[global_index_num]) == 0 or not contains_value(x, filter_row[global_index_num]):
...
root = tk.Tk()
root.geometry("1000x530+" + str(int((screen_width / 2) - 500)) + "+" + str(int((screen_height / 2) - 215)))
root.title('DataView')
...
filter_window = tk.Tk()
filter_window.geometry("300x400+" + str(int((screen_width / 2) - 150)) + "+" + str(int((screen_height / 2) - 200)))
...
e1[-1].grid(row=1, column=0, sticky=W, padx=10)
...
value_frame.grid(row=3, column=0, padx=5)
...
remove_button.grid(row=4, column=0, pady=5, sticky="news")
remove_button.config(state="disabled")
ok_button[-1].grid(row=5, column=0, pady=5, sticky="news")
cancel.grid(row=6, column=0, pady=5, sticky="news")
...
container = tk.Frame(window_frame, width=1000, height=500)
